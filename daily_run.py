...
ALL_INDICES_ENDPOINT = "https://www.nseindia.com/api/allIndices"
INDEX_NAME_MAP = {
    "NIFTY": "NIFTY 50",
...
def fetch_previous_day_ohlc(symbol):
    ...
    resp = session.get(ALL_INDICES_ENDPOINT, timeout=10)
    resp.raise_for_status()
    data = resp.json()
    rows = data.get("data", [])
    target_name = INDEX_NAME_MAP[symbol]
    row = next((r for r in rows
                if r.get("index") == target_name or r.get("indexSymbol") == target_name), None)
    if not row:
        available = ", ".join(r.get("index", r.get("indexSymbol", "?")) for r in rows[:8])
        raise RuntimeError("Could not find '" + target_name + "' in NSE's allIndices response. "
                            "Available index names seen: " + available)
    # Field names have shifted before between NSE endpoints - check a couple
    # of variants so a future rename gives a clear error instead of a KeyError.
    high = row.get("high", row.get("dayHigh"))
    low = row.get("low", row.get("dayLow"))
    prev_close = row.get("previousClose")
    if high is None or low is None or prev_close is None:
        raise RuntimeError("Unexpected row shape for " + target_name + " - keys present: " +
                            ", ".join(row.keys()))
...
def send_telegram_message(text):
    ...
    if not resp.ok:
        # Embed Telegram's exact reason directly in the crash message itself,
        # so it shows up in the red traceback line with nothing to scroll for.
        raise RuntimeError("Telegram API error " + str(resp.status_code) + ": " + resp.text)
    return resp.json()
